  ...
  return (
    <div className="fixed top-20 right-6 z-[100]">
      <button 
        ...
        className="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400 hover:text-white transition-colors shadow-xl"
      >
        <Target size={18} className={isOpen ? 'rotate-45 transition-transform' : 'transition-transform'} />
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-12 right-0 w-64 glass-panel border border-slate-700/50 rounded-2xl p-4 shadow-2xl"
          >
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Feature Flags</p>
              ...
              {(Object.keys(flags) as Array<keyof FeatureFlags>).map(key => (
                <label key={key} className="flex items-center justify-between cursor-pointer group">
                  ...
                  <div 
                    ...
                    className={`w-8 h-4 rounded-full transition-colors relative ${flags[key] ? 'bg-amber-400' : 'bg-slate-700'}`}
                  >
                    <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-all ${flags[key] ? 'left-4.5' : 'left-0.5'}`} />
                  </div>
                </label>
              ))}
            </div>
